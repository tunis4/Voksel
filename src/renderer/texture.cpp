...
namespace renderer {
    ...
    void TextureManager::create_pending_textures() {
        std::string name;
        while (m_textures_to_create.try_dequeue(name)) {
            u32 index = m_texture_names_to_indices.at(name);
            m_textures[index]->create("res/textures/" + name + ".png");
        }
    }
    ...
    u32 TextureManager::load_texture(const std::string &name) {
        ...
        auto entry = p.first;
        m_textures.emplace_back(new Texture(m_context));
        m_textures_to_create.enqueue(name);
        return entry->second;
    }
    ...
    void Texture::create(std::string_view filename) {
        ...
        m_width = width;
        m_height = height;
        m_mip_levels = std::bit_width(std::max(m_width, m_height));
    ...
    void Texture::transition_image_layout(VkImageLayout old_layout, VkImageLayout new_layout) {
        ...
        if (old_layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
            ...
            log(LogLevel::ERROR, "Renderer/Texture", "Unsupported image layout transition");
        }
